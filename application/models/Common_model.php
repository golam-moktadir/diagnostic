...
class Common_model extends CI_Model {
    ...
    function update_about($max_id, $about) {
    ...
    function data_date_to_date_group_by($table_name, $distinct_column, $date_column, $date_from, $date_to) {
     ...
     function doc_commission_doc($date_from, $date_to, $doc_name) {
        $this->db->where('date >=', $date_from);
        $this->db->where('date<=', $date_to);
        $this->db->where('doctor_name', $doc_name);
        $this->db->group_by('invoice_no');
        $this->db->order_by('record_id');
        $query = $this->db->get('sales_test');
        return $query->result();
    }
     function doc_commission_without_doc($date_from, $date_to) {
        $this->db->where('date >=', $date_from);
        $this->db->where('date<=', $date_to);
        $this->db->group_by('invoice_no');
        $this->db->order_by('record_id');
    ...
    function find_doc_commission($doctor) {
        $this->db->where("doctor_name", $doctor);
        $this->db->order_by("record_id", 'DESC');
        $this->db->limit(1);
        $query = $this->db->get("consultancy");
        return $query->row('doctor_commission');
    }
    ...
    function group_by_usual($where_column, $where_column_value, $distinct_column, $table_name) {
        $this->db->where($where_column, $where_column_value);
        $this->db->group_by($distinct_column);
        $query = $this->db->get($table_name);
        return $query->result();
    ...
    public function single_row($array,$table){
        $this->db->where($array);
