...
class Insert extends CI_Controller {
    ...
    public function photo_gallery() {
        if ($this->session->userdata('ses_user_type') == "admin") {
            $this->form_validation->set_rules('title', 'Image Tile', 'trim|required');
            if ($this->form_validation->run() == FALSE) {
                redirect('Show_form/photo_gallery', 'refresh');
            } else {
    ...
    public function insert_contact() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $value = $this->Common_model->last_row('contact');
            if(empty($value->id)){
                $this->Common_model->insert_data('contact', $data); 
            }
            else{
                $this->Common_model->update_data_onerow('contact',$data,'id',$value->id);
            }
            $this->session->set_flashdata('success','Contact Information Inserted Successfully');
    ...
    public function single_page_content() {
        if ($this->session->userdata('ses_user_type') == "admin") {
            ...
            if ($this->form_validation->run() == FALSE) {
                redirect('Show_form/single_page_content', 'refresh');
            } else {
...
//            }
    ...
    public function about_us(){
        ...
        if($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('error','Please Fill up the this Field');
            redirect(base_url().'Show_form/about_us');
        } 
        else{
            $data = ["about" => $this->input->post('about')];
            $value = $this->Common_model->last_row('about');
           
            if(empty($value->id)){
                $this->Common_model->insert_data('about', $data); 
            }
            else{
                $this->Common_model->update_data_onerow('about',$data,'id',$value->id);
            }
            $this->session->set_flashdata('success','Successfully Done');
            redirect('Show_form/about_us', 'refresh');
        }      
    }
    public function news(){
       ...
       if($this->form_validation->run() == false){
            $this->session->set_flashdata('error','Please Fill Out this field');
            redirect(base_url().'Show_form/insert_news');
       }
       else{
            $data = ["news" => $this->input->post('news')];
            $value = $this->Common_model->last_row('new_msg');
           
            if(empty($value->record_id)){
                $this->Common_model->insert_data('new_msg', $data); 
            }
            else{
                $this->Common_model->update_data_onerow('new_msg',$data,'record_id',$value->record_id);
            }
            $this->session->set_flashdata('success','Successfully Done');
            redirect(base_url().'Show_form/insert_news');
       }
